...
def create_helper_dicts(qa, test_all, test_rel, test_irrel):
    ...
    for q in test_all:
        ...
        if len(qa[qid].get('answer_set', [])):
            test_all_result_dict[qid] = pred in qa[qid]['answer_set']
        else:
    ...
    for q in test_rel:
        ...
        # dict for correctness of predicted answer
        if len(qa[qid].get('answer_set', [])):
            test_rel_result_dict[qid] = pred in qa[qid]['answer_set']
        else:
    ...
    for q in test_irrel:
        ...
        # dict for correctness of predicted answer
        if len(qa[qid].get('answer_set', [])):
            test_irrel_result_dict[qid] = pred in qa[qid]['answer_set']
        else:
...
def calculate_FPVG(test_all_input, test_rel_input, test_irrel_input, data_dir, qa_input='', verbose=2,
                   ...
                   label2ans=None, content_based_rel=False):
    ...
    acc_all_before_filter = np.average(list(test_all_result_dict.values())) * 100
    acc_all_count = len(test_all_result_dict)
    acc_rel_before_filter = np.average(list(test_rel_result_dict.values())) * 100
    acc_irrel_before_filter = np.average(list(test_irrel_result_dict.values())) * 100
    if verbose >= 1:
    ...
    for qa_scname in list(qa_subcategories):
        ...
        raw_results = [acc_all_before_filter, acc_rel_before_filter, acc_irrel_before_filter,
                           ...
                           ggc_rate, ggw_rate, bgc_rate, bgw_rate, gg_rate, bg_rate, (ggc_rate/gg_rate)*100, (bgc_rate/bg_rate)*100,
                       ((ggc_rate/gg_rate)/(bgc_rate/bg_rate))*100, (ggc_rate/bgc_rate)*100, (ggc_rate/acc_all_after_filter)*100]
        ...
        printed_results_dict = {"ACC all": (printed_results[0], acc_all_count),
                                ...
                                # "FPVG neg_effect": printed_results[13],
                                # "FPVG lev": printed_results[14],
                                # "FPVG lev2": printed_results[15],
                                # "FPVG ggc_acc": printed_results[16]
                                }
