...
from components.fc import FCNet, GTH, FCNet_ramen, FCNet_linear
...
class UpDn_ramen_transfer(nn.Module):
    def __init__(self, opt):
        super(UpDn_ramen_transfer, self).__init__()
        num_hid = opt.num_hid
        self.opt = opt
        ...
        self.q_net = FCNet([self.q_emb.num_hid, num_hid], dropout=0.0)
        self.gv_net = FCNet([num_hid, num_hid], dropout=0.0)
        # self.gv_linear = FCNet_linear([self.full_v_dim, num_hid], dropout=0.0)
        self.gv_linear = FCNet([self.full_v_dim, num_hid], dropout=0.0)


        # self.gv_att = UpDnAttention_ramen(v_dim=self.full_v_dim, q_dim=self.q_emb.num_hid, num_hid=num_hid, dropout=0.2)
        self.gv_att = UpDnAttention_ramen(v_dim=num_hid, q_dim=self.q_emb.num_hid, num_hid=num_hid, dropout=0.2)

        self.classifier = SimpleClassifier(in_dim=num_hid, hid_dim=2 * num_hid, out_dim=opt.num_ans_candidates,
                                           dropout=0.5)
    ...
    def forward(self, q, gv):
        ...
        # v embedding
        gv_linear = self.gv_linear(gv)  # [batch, 1, v_dim]

        # att embedding
        # att_gv = self.gv_att(gv, q_emb)  # [batch, 1, v_dim]
        # gv_emb = (att_gv * gv).sum(1)

        att_gv = self.gv_att(gv_linear, q_emb)  # [batch, 1, v_dim]
        gv_emb = (att_gv * gv_linear).sum(1)
    ...
    class UpDn_ramen_text(nn.Module):
        def __init__(self, opt):
            super(UpDn_ramen_text, self).__init__()
            num_hid = opt.num_hid
            self.opt = opt
            ...
            self.w_emb = WordEmbedding(opt.ntokens, emb_dim=300, dropout=0.0)
            self.w_emb.init_embedding(f'{opt.data_dir}/glove6b_init_300d.npy')
            self.wo_emb = WordEmbedding(opt.ntokens, emb_dim=300, dropout=0.0)
            self.wo_emb.init_embedding(f'{opt.data_dir}/glove6b_init_300d.npy')
            self.q_emb = QuestionEmbedding(in_dim=300, num_hid=num_hid)
            self.o_emb = QuestionEmbedding(in_dim=300, num_hid=num_hid)

            self.q_net = FCNet([self.q_emb.num_hid, num_hid], dropout=0.0)
            self.o_net = FCNet([self.q_emb.num_hid, num_hid], dropout=0.0)
            ...
            # self.gv_att = UpDnAttention_ramen(v_dim=self.full_v_dim, q_dim=self.q_emb.num_hid, num_hid=num_hid, dropout=0.2)
            self.gv_att = UpDnAttention_ramen(v_dim=self.q_emb.num_hid, q_dim=self.q_emb.num_hid, num_hid=num_hid, dropout=0.2)
        ...
        def forward(self, q, gv):
            ...
            w_emb = self.w_emb(q)
            q_emb = self.q_emb(w_emb)  # run GRU on word embeddings [batch, q_dim]
            wo_emb = self.wo_emb(gv)
            o_emb = self.o_emb(wo_emb)  # run GRU on word embeddings [batch, 1, v_dim]


            # v embedding
            # gv_linear = self.gv_linear(o_emb)  # [batch, 1, v_dim]

            # att embedding
            # att_gv = self.gv_att(gv, q_emb)  # [batch, 1, v_dim]
            # gv_emb = (att_gv * gv).sum(1)

            att_gv = self.gv_att(o_emb, q_emb)  # [batch, 1, v_dim]
            gv_emb = (att_gv * gv_linear).sum(1)
