...
def run(model, train_loader, eval_loader_all, eval_loader_all_2, opt, train_set=None, eval_set=None):
    ...
    if opt.model_type == 'lxmert': 
        ...
        if opt.lxmert_small_model:
            # for small lxmert model (trained only on visfis):
            ...
            if opt.visfis_all:
                if opt.feature_name[:6] == 'oracle':
                    tokenizer = transformers.LxmertTokenizer.from_pretrained(
                        os.path.join(opt.data_dir, 'lxmert_pretrained_models', "lxmertPretrain_ORA/"),
                        config=lxmert_config)
                elif opt.infusion:
                    tokenizer = transformers.LxmertTokenizer.from_pretrained(
                        os.path.join(opt.data_dir, 'lxmert_pretrained_models', "lxmertPretrain_INF/"),
                        config=lxmert_config)
                else:
                    tokenizer = transformers.LxmertTokenizer.from_pretrained(
                        os.path.join(opt.data_dir, 'lxmert_pretrained_models', "lxmertPretrain_DET/"),
                        config=lxmert_config)
    ...
    if opt.load_checkpoint_path is not None and len(opt.load_checkpoint_path) > 0:
        ...
        if "epoch" in ckpt:
            ...
            states_ = ckpt
        # me:
        # if oracle transfer is used, need to skip loading entry layer for vision because of dim mismatch
        # this layer will be retrained
        if opt.model_type == "updn_ramen_transfer" and not opt.ACC_only and not opt.no_init_vision:
            print("Loading model from checkpoint and re-initializing vision layer weights!")
            # not loading the first vision layer weights
            pretrained_dict = states_ #pretrained model keys
            processed_dict = {}
            for k in pretrained_dict.keys():
                decomposed_key = k.split(".")
                if ("gv_linear" in decomposed_key):
                    continue
                else:
                    processed_dict[k] = pretrained_dict[k]
            model.load_state_dict(processed_dict, strict=False)  # strict here is important since the heads layers are missing
            # from the state, we don't want this line to raise an error but load the present keys anyway.
        else:
            print("Loading model from checkpoint and using as-is!")
            # if regular model, without oracle transfer
